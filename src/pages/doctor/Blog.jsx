...
import { Search, Calendar, User, Clock, ArrowRight, Stethoscope, FileText, TrendingUp, Award } from "lucide-react";
  ...
  const fetchBlogs = async () => {
    try {
      ...
      if (response.status === 1 || response.blogs) {
        const doctorBlogs = (response.blogs || []).filter(blog => blog.type === "Doctor");
        setBlogs(doctorBlogs);
        setFilteredBlogs(doctorBlogs);
      }
    } catch (error) {
